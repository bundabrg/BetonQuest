 ...
 */
package pl.betoncraft.betonquest.v1_13_R1.compatibility.citizens;

import pl.betoncraft.betonquest.BetonQuest;
import pl.betoncraft.betonquest.v1_13_R1.compatibility.LocalCompatibility;
import pl.betoncraft.betonquest.compatibility.Integrator;
import pl.betoncraft.betonquest.v1_13_R1.compatibility.protocollib.NPCHider;
import pl.betoncraft.betonquest.v1_13_R1.compatibility.protocollib.UpdateVisibilityNowEvent;
...
public class CitizensIntegrator implements Integrator {
    ...
    public void hook() {
        ...
        if (LocalCompatibility.getInstance().getHooked().contains("EffectLib"))
            new CitizensParticle();

        // if ProtocolLib is hooked, start NPCHider
        if (LocalCompatibility.getInstance().getHooked().contains("ProtocolLib")) {
            NPCHider.start();
    ...
    @Override
    public void reload() {
        if (LocalCompatibility.getInstance().getHooked().containsAll(Arrays.asList("Citizens", "EffectLib"))) {
